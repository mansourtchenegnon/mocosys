...
from utility.datasets.skeleton import Skeleton
from utility.datasets.mocap_dataset import MocapDataset
from utility.datasets.camera import normalize_screen_coordinates, world_to_camera
...
def get_human_36m_skeleton():
    h36m_skeleton = Skeleton(parents=[-1,  0,  1,  2,  3,  4,  0,  6,  7,  8,  9,  0, 11, 12, 13, 14, 12,
       16, 17, 18, 19, 20, 19, 22, 12, 24, 25, 26, 27, 28, 27, 30],
       joints_left=[6, 7, 8, 9, 10, 16, 17, 18, 19, 20, 21, 22, 23],
       joints_right=[1, 2, 3, 4, 5, 24, 25, 26, 27, 28, 29, 30, 31])
    return h36m_skeleton
...
class Human36mDataset(MocapDataset):
    def __init__(self, path="./data/human36m/data_3d_h36m.npz", keypoints_path="./data/human36m/data_2d_h36m_gt.npz", remove_static_joints=True):
        ...
        """
        super().__init__(fps=50, skeleton=get_human_36m_skeleton())
        ...
        # Load 2D keypoints
        contents = np.load(keypoints_path, allow_pickle=True)
        self._keypoints_metadata = contents['metadata'].item()
        keypoints_symmetry = self._keypoints_metadata['keypoints_symmetry']
        self._kps_left, self._kps_right = list(keypoints_symmetry[0]), list(keypoints_symmetry[1])
        self._joints_left, self._joints_right = list(self.skeleton().joints_left()), list(self.skeleton().joints_right())
        self._keypoints = contents['positions_2d'].item()
        for subject in self.subjects():
            assert subject in self._keypoints, 'Subject {} is missing from the 2D detections dataset'.format(subject)
            for action in self._data[subject].keys():
                assert action in self._keypoints[subject], 'Action {} of subject {} is missing from the 2D detections dataset'.format(action, subject)
                if 'positions_3d' not in self._data[subject][action]:
                    continue
                    
                for cam_idx in range(len(self._keypoints[subject][action])):
                    
                    # We check for >= instead of == because some videos in H3.6M contain extra frames
                    mocap_length = self._data[subject][action]['positions_3d'][cam_idx].shape[0]
                    assert self._keypoints[subject][action][cam_idx].shape[0] >= mocap_length
                    
                    if self._keypoints[subject][action][cam_idx].shape[0] > mocap_length:
                        # Shorten sequence
                        self._keypoints[subject][action][cam_idx] = self._keypoints[subject][action][cam_idx][:mocap_length]

                assert len(self._keypoints[subject][action]) == len(self._data[subject][action]['positions_3d'])
                
        for subject in self._keypoints.keys():
            for action in self._keypoints[subject]:
                for cam_idx, kps in enumerate(self._keypoints[subject][action]):
                    # Normalize camera frame
                    cam = self.cameras()[subject][cam_idx]
                    kps[..., :2] = normalize_screen_coordinates(kps[..., :2], w=cam['res_w'], h=cam['res_h'])
                    self._keypoints[subject][action][cam_idx] = kps
    ...
    def fetch(self, subjects, action_filter=None, parse_3d_poses=True):
        """
        Fetches the 2D and 3D poses for the given subjects and actions.
        Parameters
        ----------
        subjects : list
            List of subjects to fetch.
        action_filter : list, optional
            List of action prefixes to filter by. The default is None.
        parse_3d_poses : bool, optional
            Whether to parse 3D poses. The default is True.
        Returns
        -------
        camera_params : list
        ...
        for subject in subjects:
            for action in self._keypoints[subject].keys():
                ...
                loaded_poses_2d = self._keypoints[subject][action]
                for i in range(len(loaded_poses_2d)): # Iterate across cameras
                    poses_2d.append(loaded_poses_2d[i])
                    
                if subject in self.cameras():
                    cams = self.cameras()[subject]
                    assert len(cams) == len(loaded_poses_2d), 'Camera count mismatch %d vs. %d' % (len(cams), len(poses_2d))
                    for cam in cams:
                        if 'intrinsic' in cam:
                            camera_params.append(cam['intrinsic'])
                    
                if parse_3d_poses and 'positions_3d' in self._data[subject][action]:
                    loaded_poses_3d = self._data[subject][action]['positions_3d']
                    assert len(loaded_poses_3d) == len(loaded_poses_2d), 'Camera count mismatch %d vs. %d' % (len(poses_3d), len(poses_2d))
                    for i in range(len(loaded_poses_3d)): # Iterate across cameras
                        poses_3d.append(loaded_poses_3d[i])
                        codenames.append("{}_{}".format(subject, action))
        ...
        return camera_params, poses_3d, poses_2d, codenames
    
    def fetch_actions(self, actions):
        """Fetches the 3D and 2D poses for the given actions.

        Parameters
        ----------
        actions : list
            List of actions to fetch.
        ...
        for subject, action in actions:
            ...
            loaded_poses_3d = self._data[subject][action]['positions_3d']
            assert len(loaded_poses_3d) == len(poses_2d), 'Camera count mismatch'
            for i in range(len(loaded_poses_3d)): # Iterate across cameras
                poses_3d.append(loaded_poses_3d[i])
                codenames.append("{}_{}".format(subject, action))
        return poses_3d, poses_2d, codenames
...
class Human36mDatasetGenerator:
    ...
    def generate(self, subjects, action_filter=None, parse_3d_poses=True):
        camera_params, poses_3d, poses_2d, codenames = self._dataset.fetch(subjects, action_filter, parse_3d_poses)
        return camera_params, poses_3d, poses_2d, codenames
